...
- **容器管理**：支持容器列表、搜索、启动、停止、重启、确认删除和日志订阅；启动、停止和重启成功后仅更新当前行状态，不重新加载列表，也不改变当前列表顺序；运行中容器删除会在确认后强制停止并删除，删除成功后以过渡动画移出列表。
...
| `StartContainer` / `StopContainer` / `RestartContainer` / `RemoveContainer` | 执行容器生命周期操作，启动、停止和重启成功后前端仅更新当前行状态且不改变当前列表顺序，删除前由前端展示确认提示 |
