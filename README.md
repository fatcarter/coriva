...
- **环境概览**：展示当前 Docker context、Docker Engine、Docker Compose V2、Docker 所在服务器关键信息、按日期分组的最近操作状态，以及通过 Docker Engine SDK 读取到的 Docker 参数列表；服务器信息固定展示 12 个字段，区域高度按固定行数计算。
- **多 context 管理**：支持左下角快速切换 Docker 连接，在设置页通过模态弹窗维护 Context，并按规范化 Host 防止重复 URI；候选连接验证成功后才会提交切换，失败不会影响当前 Context；删除连接时会先确认，成功后以过渡动画移出列表。
...
- **镜像管理**：支持镜像列表、搜索、无默认值的镜像拉取输入、按拉取任务聚合的进度条、取消拉取、按镜像配置运行容器和确认删除；删除成功后以过渡动画移出列表。
- **Compose 管理**：支持添加本地 Compose 项目、读取服务状态、启动、停止、重启和日志订阅。
- **资源管理**：提供 Docker volume 只读列表；Docker network 支持列表、搜索、容器使用数量展示、点击数量查看已连接容器并断开连接、创建、详情、连接容器、断开容器、删除和带说明提示的未使用网络清理，并支持 labels、driver options、IPAM、filter 等高级参数。
- **本地记录**：使用 SQLite 保存 Compose 项目和最近操作，最近操作仅保留最新 100 条，所有本地表均包含逻辑删除字段。
- **桌面体验**：macOS 使用保留系统交通灯的沉浸式标题栏；模态弹窗和下拉框使用统一的深色主题、键盘交互与减少动态效果适配；资源列表使用共享的左侧定位条，在鼠标跨行移动时连续跟随当前行。
...
首次启动时如果尚未选择 Coriva context，会按 `DOCKER_HOST`、`DOCKER_CONTEXT`、Docker `currentContext` 和默认 socket 解析一个只读启动连接。Coriva 不再读取 Docker CLI context 列表，也不会把 CLI contexts 自动加入 Coriva 自维护列表；需要长期保留的连接请在设置页手动新增。Coriva 切换 context 不会执行 `docker context use`，也不会修改系统 Docker CLI 的 `currentContext`。切换时会先探测候选连接，仅在探测成功后更新活动连接、Compose 环境和本地选择；探测失败时继续使用原 Context。

Context 维护入口位于设置页。新增或编辑使用统一模态弹窗，字段尺寸与网络连接弹窗保持一致。Coriva 会规范化 Host 并校验重复 URI；保存后立即执行一次连接检测，检测失败也允许保存，并记录失败信息。`unix://`、`npipe://`、`localhost`、`127.0.0.0/8` 和 `::1` 会标记为 `local bridge`，其他地址标记为 `remote bridge`。
...
| `SwitchDockerContext` | 探测并切换 Coriva 当前 Docker context；探测失败时保留原 Context，且不修改系统 Docker CLI 配置 |
...
| `ListImages` / `InspectImageRunConfig` / `RunImage` / `PullImage` / `CancelImagePull` / `RemoveImage` | 查询镜像、读取镜像默认运行配置、从镜像创建并启动容器、拉取、取消拉取或删除镜像；删除前由前端展示确认提示 |
...
| `ListVolumes` | 只读查询数据卷 |
| `ListNetworks` | 查询 Docker 网络列表，前端结合容器列表展示每个网络的已连接容器数量，并支持从数量弹窗发起断开连接操作 |
...
- `CORIVA_IMAGE_RUN`：根据镜像配置创建并启动容器。
