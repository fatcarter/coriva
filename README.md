...
- **环境概览**：展示当前 Docker context、Docker Engine、Docker Compose V2、Docker 所在服务器关键信息、最近操作状态，以及通过 Docker Engine SDK 读取到的 Docker 参数列表。
- **多 context 管理**：支持左下角快速切换 Docker 连接，在设置页维护 Context，启动时自动同步本机 Docker CLI contexts，并按规范化 Host 防止重复 URI。
- **容器管理**：支持容器列表、搜索、启动、停止、重启、确认删除和日志订阅；运行中容器删除会在确认后强制停止并删除。
- **镜像管理**：支持镜像列表、搜索、无默认值的镜像拉取输入、拉取进度订阅和确认删除。
- **Compose 管理**：支持添加本地 Compose 项目、读取服务状态、启动、停止、重启和日志订阅。
- **资源查看**：提供 Docker volume 和 network 的只读列表。
...
首次启动时如果尚未选择 Coriva context，会按 `DOCKER_HOST`、`DOCKER_CONTEXT`、Docker `currentContext` 和默认 socket 解析一个启动连接。检测到本机 Docker CLI 时，Coriva 会读取 Docker CLI context 列表，并把尚未存在的 Host 自动加入 Coriva 自维护列表；已存在的规范化 Host 会被忽略，不覆盖用户已维护的名称、凭据或说明。Coriva 切换 context 不会执行 `docker context use`，也不会修改系统 Docker CLI 的 `currentContext`。
...
删除 Coriva 自有 Docker 连接时，前端会先展示确认提示。后端会校验 SQLite 逻辑删除实际影响行数，避免连接不存在或已删除时仍返回成功。
...
| `ListDockerContexts` | 查询 Coriva 自维护 Docker contexts，读取前会自动同步本机 Docker CLI contexts |
...
| `ListContainers` | 查询容器列表，支持搜索和是否包含停止容器 |
| `StartContainer` / `StopContainer` / `RestartContainer` / `RemoveContainer` | 执行容器生命周期操作，删除前由前端展示确认提示 |
| `StreamContainerLogs` / `StopLogStream` | 订阅或停止容器日志流 |
| `ListImages` / `PullImage` / `RemoveImage` | 查询、拉取或删除镜像，删除前由前端展示确认提示 |
...
| `ListVolumes` / `ListNetworks` | 只读查询数据卷和网络 |
